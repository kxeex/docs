...
**Features:**
- Automatic aspect ratio conversion (16:9 or 9:16)
- Formats: JPEG, PNG, WEBP
- Max size: 20 MB

**Models:** `veo-3-fast`, `veo-3-quality`

### 3. Frame-to-Frame (FL Mode)
Generate smooth transition between two frames using `start_frame` and `end_frame` parameters.

**Features:**
- Both frames are **required**
- Automatic FL model selection based on aspect ratio
- Parallel frame upload for faster processing
- Automatic aspect ratio conversion

**Models:** `veo-3-fast`, `veo-3-quality` (FL model auto-detected)
...
- Graceful degradation: works with partially loaded images (minimum 1 required)
- **Only Fast models supported** (automatically uses `veo-3-fast`)

**Models:** `veo-3-fast` only
